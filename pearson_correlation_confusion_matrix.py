...
def plot_neural_state_pearson_confusion_matrix(matrix, x_labels, y_labels, title, tick_step=10):
    ...
    plt.title(title, fontsize=20, font="Arial")
    plt.xlabel("Neural states from first distribution", fontsize=15, font="Arial")
    plt.ylabel("Neural states from second distribution", fontsize=15, font="Arial")

    x_tick_positions = np.arange(0, len(x_labels), tick_step)
    y_tick_positions = np.arange(0, len(y_labels), tick_step)

    plt.xticks(x_tick_positions, [x_labels[i] for i in x_tick_positions], rotation=45, ha="right", fontsize=15, font="Arial")
    plt.yticks(y_tick_positions, [y_labels[i] for i in y_tick_positions], fontsize=15, font="Arial")
...
def plot_neural_state_euclidean_confusion_matrix(
    ...
    plt.title(title, fontsize=30,font="Arial")
    plt.xlabel("Neural states from first distribution", fontsize=23, font="Arial")
    plt.ylabel("Neural states from second distribution", fontsize=23, font="Arial")

    x_tick_positions = np.arange(0, len(x_labels), tick_step)
    y_tick_positions = np.arange(0, len(y_labels), tick_step)

    plt.xticks(x_tick_positions,[x_labels[i] for i in x_tick_positions], rotation=45, ha="right", fontsize=45, font="Arial")

    plt.yticks(y_tick_positions, [y_labels[i] for i in y_tick_positions], fontsize=45, font="Arial")

    cbar = plt.colorbar(im)
    cbar.set_label("Euclidean distance", fontsize=23, font="Arial")
